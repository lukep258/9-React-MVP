export default function randomText(){
    const PnPtxt = `It is a truth universally acknowledged, that a single man in possession
...
`
    const PnParr = PnPtxt.split(/\s+/)
    const startI = Math.floor(Math.random()*3000)
    const returnArr = PnParr.slice(startI,startI+300)
    return returnArr
}
